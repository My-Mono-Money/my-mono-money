...
export class AddMonobankToken1657286085973 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE "monobank_token" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "token" character varying NOT NULL, "created_at" TIMESTAMP NOT NULL DEFAULT timezone('utc', now()), "updated_at" TIMESTAMP NOT NULL DEFAULT timezone('utc', now()), "space_id" uuid, CONSTRAINT "PK_8054a49ccd3fe160e0fdca03ed7" PRIMARY KEY ("id"))`,
    );
    await queryRunner.query(
